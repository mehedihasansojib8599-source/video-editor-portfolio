...
export function SectionDivider() {
  ...
  return (
    <div className="relative overflow-hidden border-y border-line bg-bg-surface">
      {/* ambient brand-color glows — smaller/softer on mobile so they don't overpower a narrow screen */}
      <div className="pointer-events-none absolute -left-32 top-0 h-48 w-48 rounded-full bg-accent/10 blur-[60px] sm:h-72 sm:w-72 sm:blur-[110px]" />
      <div className="pointer-events-none absolute -right-32 bottom-0 h-48 w-48 rounded-full bg-teal/10 blur-[60px] sm:h-72 sm:w-72 sm:blur-[110px]" />
          ...
          {highlights.map(({ icon: Icon, label, color }, i) => {
            ...
            return (
              <motion.div
                ...
                className="group flex flex-col items-center gap-2 text-center sm:gap-3"
              >
                <div
                  className={
                    'relative flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ring-1 transition-all duration-300 group-hover:scale-110 group-hover:-translate-y-1 sm:h-14 sm:w-14 ' +
                    (isTeal
                ...
                <p className="text-xs font-medium text-ink transition-colors duration-300 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-accent group-hover:to-teal sm:text-sm">
                  {label}
                </p>
              </motion.div>
            );
      ...
      <div className="relative overflow-hidden border-t border-line/70 bg-gradient-to-r from-accent/5 via-transparent to-teal/5 py-3 sm:py-4">
        <div className="absolute inset-y-0 left-0 z-10 w-16 bg-gradient-to-r from-bg-surface to-transparent sm:w-24" />
        <div className="absolute inset-y-0 right-0 z-10 w-16 bg-gradient-to-l from-bg-surface to-transparent sm:w-24" />

        <div className="flex w-max animate-marquee items-center gap-6 font-mono text-[11px] uppercase tracking-widest text-ink-faint sm:gap-10 sm:text-xs">
